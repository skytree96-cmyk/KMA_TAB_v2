...
관리자 대시보드는 현재 브라우저 세션에 저장된 실제 프로젝트 0~1개와 참여자 0~1명의 교육 전·후 응답·완료 상태만 표시합니다. 서버 DB가 없으므로 다른 브라우저·참여자·회원사 데이터는 자동 합산되지 않습니다.

공개 MVP의 교육 전 검사는 교육담당자가 프로젝트를 만든 동일 브라우저 세션에서만 시작할 수 있습니다. 프로젝트를 다른 브라우저로 전달하는 링크·코드와 서버 저장은 아직 지원하지 않습니다. 교육 후 검사는 사전검사 직후 저장한 기준파일(JSON)을 새 브라우저에서 불러와 이어갈 수 있습니다.
...
3. 참여자는 프로젝트를 만든 동일 브라우저 세션에서 교육 전 `사전검사`에 응답하고, 교육 후 현업 적용 8~10주 뒤에 같은 문항·척도로 `사후검사`에 응답합니다. 최근 8주 회상기간에 교육 전이 섞이지 않도록 하며, 두 시점에는 같은 교육 참여자 ID를 사용합니다. ID가 없어도 문항은 표시되지만 응답 저장·다음 문항 이동·검사 완료는 할 수 없습니다.
...
이 ZIP은 별도 DB·로그인 없이 즉시 공개 검토 배포가 가능합니다. 진행 중 응답과 관리자 대시보드 수치는 현재 브라우저 세션에만 유지되며 다른 브라우저나 회원사 데이터와 합산되지 않습니다. 완료한 교육 전 결과는 개인 리포트의 기준파일(JSON)로 내려받아 교육 후 검사에서 복원할 수 있지만, 파일에는 가명 ID와 문항별 응답이 있으므로 본인만 안전하게 보관해야 합니다. 개인결과 공유 동의 체크박스는 자동 전송 기능이 아니며, 다인 조직 운영과 자동 취합·장기보존에는 `database/production_schema.sql` 기반 서버 저장을 연결하고 `SECURITY.md`, `DEPLOYMENT_CHECKLIST.md`를 따라야 합니다.
