...
운영용 교육 전후 리포트 CSV는 참여자별·역량별 장형 구조이며 `participant_id`, `factor_code`, `score_1_to_5`, `project_id`, `assessment_version`, `target_level`, `assessment_date`, `session_type`, `valid_items`, `na_items`, `missing_items`와 네 전이요인 열을 사용합니다. `session_type`은 `pre` 또는 `post`이며, 두 시점에는 같은 교육 참여자 ID(`participant_id`)와 같은 문항 버전을 유지해야 합니다. 화면의 `사전·사후 CSV 양식`을 사용하면 올바른 형식으로 시작할 수 있습니다. 공개 데모에는 실제 개인정보나 기밀 응답을 업로드하지 마세요.
...
이 ZIP은 별도 DB·로그인 없이 즉시 공개 검토 배포가 가능합니다. 진행 중 응답은 브라우저 세션에만 유지됩니다. 완료한 교육 전 결과는 개인 리포트의 기준파일(JSON)로 내려받아 교육 후 검사에서 복원할 수 있지만, 파일에는 가명 ID와 문항별 응답이 있으므로 본인만 안전하게 보관해야 합니다. 다인 조직 운영과 자동 장기보존에는 `database/production_schema.sql` 기반 서버 저장을 연결하고 `SECURITY.md`, `DEPLOYMENT_CHECKLIST.md`를 따라야 합니다.
