...
| 임직원 | 진단 참여 | 본인 진단·개인 리포트·선택적 결과 공유 |
| KMA 운영자 | KMA 대시보드 | 회원사 운영상태·문항 버전·과정 매핑·감사로그 |
...
3. 임직원은 `진단 참여`에서 최근 8주 행동을 한 문항씩 응답합니다. 공개 데모의 응답은 브라우저 세션에만 임시 저장됩니다.
4. 개인은 본인 리포트를 보고, 교육담당자는 `조직 리포트`에서 익명 집계를 확인합니다.
5. 조직 리포트의 `인쇄용 리포트 HTML`을 내려받아 브라우저의 `인쇄 → PDF 저장`을 사용합니다.

화면 안의 상세 안내와 PPT 가이드는 `pages/7_user_guide.py`, `docs/TAP_빠른사용가이드_v3.pptx`에서 제공합니다.
