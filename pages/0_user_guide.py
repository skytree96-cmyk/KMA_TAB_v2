...
callout(
    "관리자 대시보드의 실제 데이터 범위",
    "관리자 대시보드는 현재 브라우저 세션에서 만든 프로젝트 0~1개와 참여자 0~1명의 교육 전·후 응답·완료 상태만 표시합니다. 서버 DB가 없으므로 다른 브라우저·참여자·회원사 데이터는 자동 합산되지 않습니다.",
    icon="i",
)

callout(
    "공개 MVP의 검사 시작 범위",
    "교육 전 검사는 교육담당자가 프로젝트를 만든 동일 브라우저 세션에서만 시작할 수 있습니다. 다른 브라우저로 프로젝트를 전달하는 링크·코드와 서버 저장은 아직 지원하지 않습니다. 교육 후 검사는 사전검사 뒤 저장한 기준파일(JSON)을 새 브라우저에서 불러와 이어갈 수 있습니다.",
    icon="!",
    tone="warn",
)
...
callout(
    "조직 결과 공개 기준",
    "조직 리포트는 역량별로 교육 전·후가 모두 유효한 짝지은 참여자 N≥5일 때만 평균과 변화량을 공개합니다. 이 기준은 개인정보 보호를 위한 최소값이며 통계적 안정성이나 교육의 인과효과를 보장하지 않습니다.",
    icon="5+",
)
...
with company_tab:
    ...
    st.markdown(
        ...
        2. **관리자 대시보드**에서 현재 브라우저에 저장된 실제 프로젝트와 교육 전·후 완료 상태를 확인합니다.
        3. **교육평가 프로젝트**에서 교육명·교육일·사전/사후 검사기간과 측정역량을 정합니다.
        4. 공개 MVP의 사전검사는 프로젝트를 만든 **동일 브라우저 세션**에서 진행합니다. 참여자에게 같은 교육 참여자 ID를 배정하고 **사전검사 → 교육·현업 적용 → 사후검사** 순서로 안내합니다.
        5. 사전·사후에는 동일한 문항·척도·버전을 유지하고, 행동 사후검사는 최근 8주 회상기간과 맞춰 기본 8~10주 후에 엽니다.
        6. 각 참여자가 **사전·사후 검사를 모두 완료한 뒤** 개인 리포트에서 내려받은 **결과 CSV**를 같은 헤더의 행 단위로 합치고, **교육 전후 리포트**의 `교육 참여자 조직결과 CSV`에 업로드합니다.
        7. 역량별 짝지은 유효 참여자 N≥5 결과에서 변화와 수행기회·전이 장애요인을 확인합니다.
        8. `인쇄용 리포트 HTML`을 내려받아 브라우저에서 **인쇄 → PDF 저장**을 선택합니다.
        """
    )
...
with participant_tab:
    ...
    st.markdown(
        ...
        2. 화면의 검사단계가 `사전검사`인지 `사후검사`인지 먼저 확인합니다. 공개 MVP의 사전검사는 프로젝트를 만든 동일 브라우저에서만 시작할 수 있습니다.
        3. 가명 **교육 참여자 ID**를 먼저 입력합니다. ID가 없을 때도 문항은 보이지만 응답 저장·다음 문항 이동·검사 완료는 할 수 없습니다.
        4. 두 검사 모두 최근 8주에 해당 행동을 얼마나 자주 했는지 1~5점으로 답합니다.
        5. 그 행동을 할 기회가 없었다면 **수행 기회 없음**을 고릅니다. 미응답·0점과는 다릅니다.
        6. 사후검사에서는 교육내용을 적용할 기회, 상사지원, 도구·권한, 장애요인도 함께 답합니다.
        7. 브라우저 세션이 달라지면 사전검사 후 저장한 **교육 전 검사 기준파일**을 `교육 전 검사 기준파일(JSON)`에서 선택해 자동 복원합니다.
        8. 개인결과 공유 동의 체크박스는 의사 표시일 뿐 자동 전송 기능이 아닙니다. 조직 집계가 필요하면 **결과 CSV**를 정해진 안전한 방법으로 교육담당자에게 전달합니다.
        9. 결과는 규준·백분위·개인순위가 아니라 본인의 사전·사후 관찰 변화입니다.
        """
    )
    if st.button("실제 검사 시작", key="guide_assessment", type="primary", width="stretch"):
