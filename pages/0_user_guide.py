...
st.markdown(
    ...
    <div class="tap-guide-grid">
      ...
      <div class="tap-guide-step"><span>4</span><b>대시보드·리포트</b><small>누적 완료현황을 새로고침하고 역량별 전·후 유효응답 N≥5 결과를 확인합니다.</small></div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
callout(
    "조직 결과 공개 기준",
    "조직 리포트는 역량별로 교육 전·후가 모두 유효한 참여자 N≥5일 때만 평균과 변화량을 공개합니다. GitHub 합성 기획검증 프로젝트의 N<5 결과는 리포트 미리보기 코드와 화면 전용 미리보기 확인을 거친 경우에만 볼 수 있으며 다운로드·인쇄할 수 없습니다. 이 기준은 개인정보 보호를 위한 최소값이며 통계적 안정성이나 교육의 인과효과를 보장하지 않습니다.",
    icon="5+",
)
...
with company_tab:
    ...
    st.markdown(
        ...
        10. 역량별 전·후 유효응답 N≥5 결과에서 변화와 수행기회·전이 장애요인을 확인하고 `인쇄용 리포트 HTML`을 내려받습니다.
        """
    )
