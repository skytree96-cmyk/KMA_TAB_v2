...
callout(
    "다른 브라우저에서 실제 검사 시작",
    "GitHub 테스트 저장소가 연결되면 교육담당자가 전달한 프로젝트 코드를 참여자 화면에 입력해 동일 문항·일정·버전으로 검사를 시작할 수 있습니다. 사전·사후 완료 결과만 저장되며 문항을 넘길 때마다 저장되지는 않습니다. 미연결 때에는 동일 브라우저에서 시작하거나 기준파일(JSON)로 사후검사를 이어갑니다.",
    icon="!",
    tone="warn",
)
...
st.markdown(
    ...
    <div class="tap-guide-grid">
      ...
      <div class="tap-guide-step"><span>2</span><b>코드로 검사 시작</b><small>참여자가 다른 브라우저에서도 프로젝트 코드와 기획검증 접속코드를 입력해 같은 검사도구를 불러옵니다.</small></div>
...
with company_tab:
    ...
    st.markdown(
        ...
        4. 참여자별로 추측하기 어려운 무작위 가명 교육 참여자 ID를 배정하고 **사전검사 → 교육·현업 적용 → 사후검사** 순서로 안내합니다.
        ...
        7. **교육 전후 리포트**에서 누적 완료 결과를 선택하거나, 각 참여자의 결과 CSV를 같은 헤더의 행 단위로 합쳐 업로드합니다.
        8. 역량별 짝지은 유효 참여자 N≥5 결과에서 변화와 수행기회·전이 장애요인을 확인하고 `인쇄용 리포트 HTML`을 내려받습니다.
        """
    )
...
with participant_tab:
    ...
    st.markdown(
        ...
        2. GitHub 테스트 저장소가 연결된 경우 교육담당자에게 받은 **프로젝트 코드**와 **기획검증 접속코드**를 입력해 검사도구를 불러옵니다. 미연결이면 프로젝트를 만든 동일 브라우저에서 시작합니다.
        3. 화면의 검사단계가 `사전검사`인지 `사후검사`인지 확인하고, 교육담당자가 무작위로 배정한 가명 **교육 참여자 ID**를 입력합니다. 이름·사번은 사용하지 않습니다. ID가 없을 때도 문항은 보이지만 응답 저장·다음 문항 이동·검사 완료는 할 수 없습니다.
