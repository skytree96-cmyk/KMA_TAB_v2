...
def render_account_guide():
    ...
    for title, description in [
        ("KMA 관리자", "회원사의 사업자등록번호 10자리를 등록하고 해당 회사의 교육담당자 계정을 발급합니다. 교육담당자 임시 비밀번호는 kma이며 첫 로그인 때 변경합니다. 회사별 프로젝트와 참여 현황을 확인할 수 있습니다."),
        ("교육담당자", "교육 일정과 측정역량을 정해 프로젝트를 만듭니다. 참여자 계정을 단건 또는 CSV로 발급하고 프로젝트에 배정합니다. 부서·직급·이메일·연락처를 함께 저장할 수 있습니다. 참여자 ID 앞에는 회사 식별정보(신규 회사는 사업자등록번호)가 자동으로 붙으며 임시 비밀번호는 kma입니다. 발급한 로그인 정보를 해당 참여자에게 전달하고 전달 완료 버튼을 누르세요."),
