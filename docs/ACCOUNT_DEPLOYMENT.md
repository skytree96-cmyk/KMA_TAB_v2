...
- 회사는 하이픈 없는 ASCII 숫자 10자리 사업자등록번호로 등록한다. 앞자리 0을 보존하며 같은 번호는 중복 등록할 수 없다. 기존 회사는 KMA 관리자가 실제 번호를 등록·수정할 수 있다.
- 로그인은 이메일 없이 지정 ID와 비밀번호를 사용한다. 교육담당자는 입력한 ID를 사용하고, 참여자는 회사 식별정보(신규 회사는 사업자등록번호)와 하이픈을 앞에 붙여 회사 간 ID 충돌을 피한다. 최종 로그인 ID는 시스템 전체에서 중복될 수 없다. 기존 ID는 자동 변경하지 않고 소속 회사는 내부 회사 ID로 관리한다.
- 비밀번호 길이는 최소 3자, 최대 128자다. 초기 관리자·신규 계정·비밀번호 변경·재설정에 같은 기준을 적용한다.
- 교육담당자·참여자의 임시 비밀번호는 단건·CSV 신규 발급 및 재설정 시 모두 `kma`로 부여한다. 모든 임시 비밀번호는 처음 사용할 때 변경하며 DB에는 비밀번호 해시를 저장한다.
- 참여자의 부서·직급·이메일·연락처는 선택 입력으로 계정에 저장한다. 이메일은 로그인 인증에 사용하지 않는다. 기존 계정에는 빈 값으로 필드를 추가하며, 회사의 계정 관리 권한 안에서만 조회한다.
- 관리자 탭과 CSV 일괄 등록 영역의 선택·열림 상태를 세션에 저장하여 업로드와 작업 후 재실행에도 유지한다.
...
문항 저장은 폼 콜백에서 최신 응답에 현재 문항을 합쳐 저장한 뒤 다음 문항을 한 번만 렌더링한다. 저장 확인 결과는 같은 계정·배정·단계의 직후 렌더링에서 한 번만 사용하고, 별도 재실행에서는 DB를 다시 조회한다. 저장 실패 시 현재 문항과 선택값을 유지한다. 검사 중에는 활성 단계만 읽고, 사후 비교 결과를 표시할 때 사전 결과를 추가 조회한다.
